...
The response from the handler will be a plain text description of the checks that were run and the results of those checks. If all the checks passed, the response code will be 200 (OK). If any checks fail, the response code will be 500 (Server Error).
...
You can change the error response code in the configuration block by setting +error_code+:
  
  IsItWorking::Handler.new do |h|
    # Use 417 (Expectation Failed) response code
    h.error_code = 417
    ...
  end
